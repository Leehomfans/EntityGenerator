    ...
    public class SqlServerEntityBuild : IEntityBuild
    {
        ...
        private string CreatModel(string tableName, List<ColumnInfo> columnInfos, string namespaceName, List<string> reference = null)
        {
            ...
            foreach (ColumnInfo item in columnInfos)
            {
                ...
                {
                    attribute.AppendSpaceLine("[Column(IsPrimaryKey = true)]", 8);
                }
                else
                {
                    attribute.AppendSpaceLine("[Column]", 8);
                }
                string typeName = TypeCast(item.TypeName);
                string nullFlag = typeName != "string" && item.Nullable ? "?" : "";
                attribute.AppendSpaceLine($"public {typeName}{nullFlag} {item.ColumnName}", 8);
            ...
            strclass.AppendSpaceLine($"[Table(Name = \"{tableName}\")]",4);
